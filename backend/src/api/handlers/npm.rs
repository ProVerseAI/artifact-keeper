...
fn build_npm_metadata_response(
    ...
    for artifact in artifacts {
        ...
        // BUG-003: prefer the standard sha512- SRI value stored at publish
        // time; artifacts published before this change have no `dist_extra`,
        // so fall back to the old (non-standard) sha256- computation for
        // those until they are republished.
        let dist_extra = artifact.metadata.as_ref().and_then(|m| m.get("dist_extra"));
        let stored_integrity = dist_extra
